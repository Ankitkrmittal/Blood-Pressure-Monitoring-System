...
class LLMAgent:
    ...
    def __init__(self, config: Optional[LLMConfig] = None) -> None:
        ...
        if ChatGoogleGenerativeAI is None or SystemMessage is None:
            logger.warning(
                "Gemini LangChain dependencies are unavailable. The assistant will use fallback replies."
            )
            return

        if not api_key:
            logger.warning("GEMINI_API_KEY not found. The assistant will use fallback replies.")
            return
    ...
    def generate_reply(
        ...
        if self.model is None or SystemMessage is None:
            logger.warning("Gemini model unavailable, using minimal fallback reply.")
